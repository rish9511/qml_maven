...
class EIC
{
    ...
    public:
        ...
        EIC *eicMerge(const vector<EIC *> &eics) {}

        void addRt(const float& rtVal) {
            rt.push_back(rtVal);
        }

        void addIntensity(const float& intensityVal) {
            intensity.push_back(intensityVal);
        }

        const std::vector<float> getRts() { return rt; }
        const std::vector<float> getIntensities() { return intensity; }
